...
def benchmark_scaling(
    ...
    CARVE_AVAILABLE_METRICS_S = [
        "ari_stability", "ari_stability_1se", "ari_stability_quant",
        
        "consensus_pac_stability", "consensus_gini_stability", "consensus_ce_stability",
    ]
    
    CARVE_AVAILABLE_METRICS_G = [
        "ari_generalizability", "ari_generalizability_1se", "ari_generalizability_quant",
    ...
    for i, x_value in enumerate(x_values):
        for true_k in true_cluster_counts:
            for seed in range(n_seeds_per_value):
                # --- 0) Set seed ---
                benchmark_seed = seed + ((true_k - min(true_cluster_counts)) * 100) + (i * 10000) + random_state
                # plotting_dict = {}  # Plotting
                ...
                # 3.3) Evaluating CARVE metrics
                for carve_metric in sorted(set(CARVE_AVAILABLE_METRICS_S).union(CARVE_AVAILABLE_METRICS_G)):
                    rule = get_rule(carve_metric)
                    measure = get_measure(carve_metric)
                    ...
                    for k in candidate_clusters:
                        ...
                        scores.append({
                            ...
                            'is_optimal': k == optimal_k,
                            'is_correct': k == true_k,
                            'metric_ari': carve_aris_s[candidate_clusters.index(k)] if carve_metric in CARVE_AVAILABLE_METRICS_S else carve_aris_g[candidate_clusters.index(k)],
                        })
