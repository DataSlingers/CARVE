...
def parse_difficulty_and_simulate(
    ...
    if (
        ...
        if difficulty_level > 0 and difficulty_level < int(round(difficulty_levels // 2)):
            ...
            difficulty_level > int(round(difficulty_levels // 2))
    ...
    *,
    ...
    if (
        ...
        if stage > 0 and stage < int(round(total_stages // 2)):
            ...
            stage > int(round(total_stages // 2))
            and stage < (total_stages - 1)
        ):
            # Linear interpolation between 'middle' and 'end' settings
            frac = (stage - int(round(total_stages // 2))) / (total_stages - 1 - int(round(total_stages // 2)))
            middle_settings = settings_by_k['middle']
