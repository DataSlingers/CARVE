...
the R companion to the Python [`carve`](https://github.com/tzUNC/ClusteringValidation)
package.
...
* **Stability** — the reproducibility of cluster assignments under
  data perturbation, summarised through ARI, consensus PAC, Gini, and
  cross-entropy.
* **Generalizability** — the agreement between held-out clusterings
  and the predictions of a classifier trained on the in-sample
  clustering.
...
CARVE depends only on CRAN packages (`R6`, `clue`, `ranger`,
`RSpectra`, `FNN`, `Matrix`, `ggplot2`, `patchwork`, `furrr`,
`progressr`). `Seurat`, `SingleCellExperiment`, and `reticulate` are
optional and only loaded when used.
...
The S3 entry point `carve()` accepts a matrix, a numeric data frame,
or — when the optional packages are installed — a `Seurat` or
